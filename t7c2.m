...
if ~exist('imds','var')
    ...
    %match_labels2;
    save('myImds7c2.mat','imds')
end

tbl = countEachLabel(imds)

if ~exist('trainingSet','var') || ~exist('validationSet','var') || ~exist('testSet','var')
    [trainingSet,validationSet, testSet] = splitEachLabel(imds,... %0.8, 0.1, 0.1,399,49,49
        90,10,10,'randomized');
    save('mySets7c2.mat','trainingSet','validationSet','testSet')
end
...
layers = [
    imageInputLayer([48 64 1],'Normalization', ...
    'none')
    %'zerocenter')
    %warstwa 1
    %conv1
...
%     convolution2dLayer([5 5],16,'Stride',[1 1],'Padding',0)
%     reluLayer
%     maxPooling2dLayer([3 3],'Stride',2)
%     crossChannelNormalizationLayer(5,'K',2)
